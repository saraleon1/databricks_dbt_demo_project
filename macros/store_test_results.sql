...
{% macro store_test_results(results) %}
  ...
  {% for result in results if result.node.resource_type == 'test' %}
    ...
    select
      '{{ test_name }}'::string as test_name,
      '{{ result.node.name }}'::string as test_name_long,
      '{{ test_type}}'::string as test_type,
      '{{ process_refs(result.node.refs) }}'::string as model_refs,
      '{{ process_refs(result.node.sources, is_src=true) }}'::string as source_refs,
      '{{ result.node.config.severity }}'::string as test_severity_config,
      '{{ result.execution_time }}'::string as execution_time_seconds,
      '{{ result.status }}'::string as test_result,
      '{{ result.failures }}'::string as failures,
      '{{ result.node.original_file_path }}'::string as file_test_defined,
